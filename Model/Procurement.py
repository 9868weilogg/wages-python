...
# global_naming_keymap = {
# 	'facility_type': 'facility_type',
# 	'facility_code': 'facility_name',
# 	'drug_code': 'drug_name',
# 	'budget_type_code': 'budget_type_name',
			...
			if value_ref[idx] not in check_temp_result[po_ref[idx]]:
				if idx == len(global_process_order[report_name]) - 1:
					obj_ = {
						...
						'name': name_ref[idx],
						'code': value_ref[idx],
						'min_unit_price': float(row['min_unit_price']),
						'max_unit_price': float(row['max_unit_price']),
						'e_p_approved_quantity': float(row['e_p_approved_quantity']),
						'purchase_amount': float(row['purchase_amount']),
						'item_packaging_name': row['item_packaging_name'],
						'item_packaging_seq_no': row['item_packaging_seq_no'],
					};
				else:
					obj_ = {
						'id': fn.convertToSnakecase(value_ref[idx]),
...
def groupData(params, data):
	...
	joined_ = [];
	joined_columns_list = [key_to_join[0]];
	data['e_p_approved_quantity'] = pd.to_numeric(data['e_p_approved_quantity']);
	...
	last_key = joined_columns_list[-1];
	# Logger.v('last_key', last_key);
	grouped_df = data.groupby([last_key])[['e_p_approved_quantity', 'purchase_amount']].apply(sum).sort_values(by='purchase_amount', ascending=False).reset_index();
...
def toOutputStructure(params, data):
	...
	Logger.v('min_purchase_amount', min_purchase_amount, type(min_purchase_amount));
	report_name = Report.getReportName(params);
						...
						if idx_count < len(process_orders) - 1:
							...
							for child_key in key3_data:
								...
								if row['purchase_amount'] > min_purchase_amount:
									obj_ = {
										'e_p_approved_quantity': row['e_p_approved_quantity'],
